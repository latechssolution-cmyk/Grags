    ...
    <AnimatePresence>
      {order && (
        <>
          ...
          >
            <div className="bg-[#0a0a0a] border border-white/10 w-full max-w-lg relative">
              {/* Header */}
              <div className="flex items-center justify-between px-6 py-5 border-b border-white/10">
                <div>
                  <p className="text-xs text-white/40 tracking-widest uppercase mb-1">Order</p>
                  <h2 className="text-lg font-medium tracking-wider">{order.id}</h2>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`px-2.5 py-1 rounded text-xs font-medium ${STATUS_COLORS[order.status] || "bg-white/10 text-white"}`}>
                    {order.status}
                  </span>
              ...
              <div className="px-6 py-5 space-y-5">
                ...
                <Section title="Payment" icon={CreditCard}>
                  ...
                  {!order.sameAsBilling && (
                    <>
                      <div className="text-white/30 text-xs pt-1">Billing Address</div>
                ...
                <Section title="Items" icon={Package}>
                  {order.products.map((p, i) => (
                    <div key={i} className="flex justify-between gap-4 py-1 border-b border-white/5 last:border-0">
                      <div>
                        <p>{p.name}</p>
                ...
                <Section title="Summary" icon={Tag}>
                  ...
                  <div className="flex justify-between gap-4 pt-2 border-t border-white/10 font-medium">
                    <span>Total</span>
